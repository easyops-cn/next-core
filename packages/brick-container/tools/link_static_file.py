  ...
  for dependency_name, dependency_info in dependencies_path_tree.items():
    ...
    if is_sa_na_dep_dir_present:
      ...
      try:
        ...
        raise RuntimeError("stanalone-na size optimization: handle standalone-na:{} dependencies info err: {}...".format(app_id, e))
...
def _link_static_file_to_sa_na(files, file_path_base, dependency_dir_inside_base, dependency_dir_public):
  for filename in files:
    ...
    try:
      ...
      raise RuntimeError("link file(src:{} dst: {}) err: {}".format(file_path, link_file_path, e))
...
if __name__ == "__main__":
  ...
  # 硬链依赖APP
  link_dependency_static_file(install_path, plugin_name, version, is_sa_na_dep_dir_present)
