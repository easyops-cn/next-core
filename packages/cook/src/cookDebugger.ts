...
export interface CookDebugger {
  ...
  setBreakpoints?(lineNumbers: number[]): void;
  stepContinue?(): void;
  stepOver?(): void;
  stepInto?(): void;
  stepOut?(): void;
...
export function cookDebugger(
  ...
  function getVariables(): unknown {
    // return getRunningContext().LexicalEnvironment._debug_only_GetBindingMap();
    const runningContext = getRunningContext();
    const local = runningContext !== rootContext ? runningContext.LexicalEnvironment._debug_only_GetBindingMap() : null;
    const closures = [];
    for (let i = executionContextStack.length - 2; i >= 0; i--) {
      const closure = executionContextStack[i];
      if (closure !== rootContext) {
        closures.push(closure.LexicalEnvironment._debug_only_GetBindingMap());
      }
    }
    return {
      local,
      closures,
      global: rootContext.LexicalEnvironment._debug_only_GetBindingMap(),
    };
  }
