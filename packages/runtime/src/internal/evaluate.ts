...
type EvaluateRuntimeContext = any;
...
export async function evaluate(
  ...
  if (attemptToVisitState) {
    ...
    let stateStore: any;
    await stateStore.waitFor(usedStates);
  }

  if (attemptToVisitGlobals.has("PROCESSORS")) {
    const usedProcessors = strictCollectMemberUsage(raw, "PROCESSORS");
    const promise = enqueueStableLoadProcessors(
      usedProcessors,
